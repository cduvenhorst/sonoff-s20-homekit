...
void button_intr_callback(uint8_t gpio)
{
	...
	if ((now - button->last_event_time) * portTICK_PERIOD_MS <
	...
	if (gpio_read(button->gpio_num) == button->pressed_value) {
		...
		if ((now - button->last_press_time) * portTICK_PERIOD_MS >
...
int button_create(const uint8_t gpio_num, bool pressed_value,
		  uint16_t long_press_time, button_callback_fn callback)
{
	...
	gpio_set_interrupt(button->gpio_num, GPIO_INTTYPE_EDGE_ANY,
			   button_intr_callback);
